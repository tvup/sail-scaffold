...
@section('title', 'Composer Packages')

@section('content')
    <h1 class="text-2xl font-bold text-cyan-400 mb-6">Composer Packages</h1>

    {{-- Add new --}}
    <form action="{{ route('admin.composer-packages.store') }}" method="POST" class="bg-gray-800 rounded-lg p-6 border border-gray-700 mb-8">
        @csrf
        <div class="flex items-end gap-4">
            <div class="flex-1">
                <label for="package" class="block text-sm text-gray-400 mb-1">Package</label>
                <input type="text" name="package" id="package" required placeholder="e.g. laravel/telescope" class="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white placeholder-gray-500 focus:border-cyan-500 focus:outline-none">
            </div>
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 text-center">
                            <span class="px-2 py-1 rounded text-xs {{ $package->dev ? 'bg-yellow-900/50 text-yellow-400' : 'bg-blue-900/50 text-blue-400' }}">
                                {{ $package->dev ? 'dev' : 'require' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <form action="{{ route('admin.composer-packages.toggle', $package) }}" method="POST" class="inline">
                                @csrf
                                @method('PATCH')
                        ...
                        <td class="px-6 py-4 text-right">
                            <form action="{{ route('admin.composer-packages.destroy', $package) }}" method="POST" class="inline" onsubmit="return confirm('Delete this package?')">
                                @csrf
                                @method('DELETE')
