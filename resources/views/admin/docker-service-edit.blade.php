...
@section('title', 'Edit Docker Service — ' . $service->name)

@section('content')
    <div class="mb-4">
        <a href="{{ route('admin.docker-services') }}" class="text-gray-400 hover:text-cyan-400 text-sm">&larr; Back to Docker Services</a>
    </div>

    <h1 class="text-2xl font-bold text-cyan-400 mb-6">Edit: {{ $service->name }}</h1>

    <form action="{{ route('admin.docker-services.update', $service) }}" method="POST" class="bg-gray-800 rounded-lg p-6 border border-gray-700 space-y-4">
        @csrf
        @method('PUT')
        <div>
        ...
        <div>
            ...
            <textarea name="config" id="config" rows="12" required class="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white placeholder-gray-500 focus:border-cyan-500 focus:outline-none font-mono text-sm">{{ $service->config }}</textarea>
        </div>
        <div>
    ...
    <form action="{{ route('admin.docker-services.destroy', $service) }}" method="POST" class="mt-4" onsubmit="return confirm('Delete this Docker service?')">
        @csrf
        @method('DELETE')
